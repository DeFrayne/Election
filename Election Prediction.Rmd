...
##########################################
# Possible new Variables for State Model #
##########################################
Unemployment <- read.csv("/Users/lizzy/Desktop/Project/Election-master/Data/State Data/Unemployment Data/Unemployment.csv", stringsAsFactors=FALSE)
Unemployment_2020 <- read.csv("/Users/lizzy/Desktop/Project/Election-master/Data/State Data/Unemployment Data/Unemployment_2020.csv", stringsAsFactors=FALSE)
Density <- read.csv("/Users/lizzy/Desktop/Project/Election-master/Data/State Data/Population Density /density_1910_2020_2.csv", stringsAsFactors = FALSE, skip=3)
StateIncome <- read.csv("/Users/lizzy/Desktop/Project/Election-master/Data/State Data/Median Income Data/Median Income.csv", stringsAsFactors = FALSE, header=TRUE, skip=4)


###############
# STATE MODEL #
###############
#original Time for Change model 
...
#Create the regression model (Note that juneapp is no longer sig at state level)
Train_Control <- trainControl(method="cv", number=10)
State_Model <- train(incvote ~ q2gdp + juneapp + inc1,
                             data = State_Data,
                             trControl = Train_Control,
                             #tuneGrid = Tune_Grid, for glmnet
                             method="glm")
#info about original model
summary(New_State_Model)
...
# Remove all data except unemployment from october of election year
Unemp_Data <- Unemployment %>%
  filter(grepl("Oct", Month)) %>%
  mutate(year = as.integer(str_sub(Month,-4,-1))) %>%
  filter(year %in% years) %>%
  relocate(year) %>%
  select(-contains("Month")) %>%
  pivot_longer(!year, names_to="state", values_to = "Unemp_Rate") 

NewUnemp2020 <- data.frame("year"= rep(2020, 51), "state" = Unemployment_2020$State, "Unemp_Rate" =Unemployment_2020$October.2020 )

Unemp_Data <- rbind(Unemp_Data, NewUnemp2020)
  

# Create new column to calculate change in unemployment since
# previous election year
Unemp_Data_Other <- Unemp_Data
Unemp_Data <- Unemp_Data %>%
  mutate(Previous=(year-4)) %>%
  left_join(Unemp_Data, Unemp_Data_Other, by = c("Previous"= "year", "state"="state")) %>%
  select(year, state, Unemp_Rate.x, Unemp_Rate.y) %>%
  rename("Previous_Unemp"=Unemp_Rate.y) %>%
  mutate(Change = (Unemp_Rate.x - Previous_Unemp)/Previous_Unemp) %>%
  filter(year!=1976)

#Isolate 2020 data to check model accuracty later
UnempChange2020 = Unemp_Data %>%
  filter(year == 2020)

# Combine new Unemployment data with original data
State_Data_PlusUnemp <- inner_join(State_Data, Unemp_Data)
...
#Create the regression model with change in Unemployment
State_Model_UnempChange <- train(incvote ~ q2gdp + juneapp + inc1 + Change,
                             data = State_Data_PlusUnemp,
                             trControl = Train_Control,
                             #tuneGrid = Tune_Grid, for glmnet
                             method="glm")
# Info about model with Unemployment Rate Change
# Unemp Change is significant! 
summary(State_Model_UnempChange)
...
#remove commas from characture and convert to numeric
Density$X1950 = as.numeric(gsub("," ,"", Density$X1950))
Density$X1960 = as.numeric(gsub("," ,"", Density$X1960))
Density$X1970 = as.numeric(gsub("," ,"", Density$X1970)) 
Density$X1980 = as.numeric(gsub("," ,"", Density$X1980)) 
Density$X1990 = as.numeric(gsub("," ,"", Density$X1990)) 
Density$X2000 = as.numeric(gsub("," ,"", Density$X2000)) 
Density$X2010 = as.numeric(gsub("," ,"", Density$X2010)) 
Density$X2020 = as.numeric(gsub("," ,"", Density$X2020)) 


# create regression to Interpolate years missing from censuse data
Xvalues <- c(1950, 1960, 1970, 1980, 1990, 2000, 2010, 2020)
ElectionYears <- c(seq(1976, 2020, by=4))
NewYears <- unique(data.frame("NewYears"=sort(c(Xvalues, ElectionYears))))

#Initiate Parallel Processing
cl <- makeCluster(detectCores()*0.5)
registerDoParallel(cl)

# loop through this for interpolation for each state
for (i in 1:52){
  DenState       <- as.numeric(Density[i, 2:9])
  df             <- data.frame(cbind(Xvalues, DenState, row.names = NULL)) 
  df             <- left_join(NewYears, df, by = c("NewYears"="Xvalues"))

  Train_Control  <- trainControl(method="cv", number=2)
  linear.model   <- train(DenState ~ NewYears, data =  df %>% na.omit, 
                      method = 'glm', trControl = Train_Control)
  M              <- is.na(df$DenState)
  df$DenState[M] <- predict(linear.model, newdata = select(df, NewYears, DenState) %>% filter(M))
  
  if (i==1){
    StateVariable = rep(Density[1,1], 17)
    NewDensity = cbind(StateVariable, df)
  }else{
    StateVariable = rep(Density[i,1], 17)
    df = cbind(StateVariable, df)
    NewDensity = rbind(NewDensity, df)
...
# Clean up new data, only keep election year data, remove PR, 
# add in state abbreviations
NewDensity1 = NewDensity %>%
  filter(NewYears %in% ElectionYears) %>%
  filter(NewYears != 1976) %>%
  filter(StateVariable != "Puerto Rico") %>%
  mutate(state=state2abbr(StateVariable)) %>%
  mutate(year = NewYears) %>%
  select(year, state, DenState) 

#Isolate 2020 data to check model accuracty later
NewDensity2020 = NewDensity1 %>%
  filter(year == 2020)

# Add new variable to dataset
State_Data_PlusDens<- inner_join(New_Data, NewDensity1)

# run model again with new variable
Train_Control <- trainControl(method="cv", number=10)
...
#Create the regression model including Density
# Note that density is not significant
State_Model_Dens <- train(incvote ~ q2gdp  + inc1 + Change + DenState,
                         data = State_Data_PlusDens,
                         trControl = Train_Control,
                         #tuneGrid = Tune_Grid, for glmnet
                         method="glm")
# info on new model
summary(State_Model_Dens)
...
# Need to find source for 2020 data

#Format data to add to previous data set
StateIncome$State[9] = "Distric of Columbia"

#remove commas from characture and convert to numeric
StateIncome$X1984 = as.numeric(gsub("," ,"", StateIncome$X1984))
StateIncome$X1988 = as.numeric(gsub("," ,"", StateIncome$X1988))
StateIncome$X1992 = as.numeric(gsub("," ,"", StateIncome$X1992)) 
StateIncome$X1996 = as.numeric(gsub("," ,"", StateIncome$X1996)) 
StateIncome$X2000 = as.numeric(gsub("," ,"", StateIncome$X2000)) 
StateIncome$X2004 = as.numeric(gsub("," ,"", StateIncome$X2004)) 
StateIncome$X2008 = as.numeric(gsub("," ,"", StateIncome$X2008)) 
StateIncome$X2012 = as.numeric(gsub("," ,"", StateIncome$X2012)) 
StateIncome$X2016 = as.numeric(gsub("," ,"", StateIncome$X2016)) 

MedianIncome <- data.frame("state" = rep(state2abbr(StateIncome$State), 9),
                           "year" = c(rep(1984, 51), rep(1988, 51), rep(1992, 51), 
                                      rep(1996, 51), rep(2000, 51), rep(2004, 51),
                                      rep(2008, 51), rep(2012, 51),
                                      rep(2016, 51)),
                             "MedIncome" = c(StateIncome$X1984, StateIncome$X1988,
                                             StateIncome$X1992, StateIncome$X1996, 
                                             StateIncome$X2000, StateIncome$X2004, 
                                             StateIncome$X2008, StateIncome$X2012,
                                             StateIncome$X2016), 
                           stringsAsFactors = FALSE)

        
                          
# Combine new variable with state data
State_Data_PlusMedIncome<- inner_join(State_Data_PlusDens, MedianIncome)
...
# Create the regression model, note that MedIncome is significant
# in model without density, but g2gdp is no longer significant 
# Lower AIC than model including density
State_Model_MedIncome <- train(incvote ~ q2gdp  + inc1 + Change  + MedIncome,
                         data = State_Data_PlusMedIncome,
                         trControl = Train_Control,
                         #tuneGrid = Tune_Grid, for glmnet
                         method="glm")
# info on new model
summary(State_Model_MedIncome)
...
#############################################
# Possible new Variables for National Model #
#############################################
NationalMeanMedian <- read.csv(url("https://raw.githubusercontent.com/DeFrayne/SQL-Summer-Bridge/master/incomeInequality.csv"))


# Original Time for Change Model (no odded variables)
# Get data in correct format for processing 
#Remove 2016 Data
...
################################
# Format Data by isolating election years and mean.median measure
ElectionYears <- c(seq(1976, 2020, by=4))
Income = NationalMeanMedian %>%
  select(Year, mean.median) %>%
  filter(Year %in% ElectionYears)

# add to original data set
National_Data_PlusIncome<- inner_join(Time_For_Change_Data, Income, by=c("year"="Year"))

#Initiate Parallel Processing
cl <- makeCluster(detectCores()*0.5)
registerDoParallel(cl)

#Create the regression model with Income measure 
National_Model_PlusIncome <- train(incvote ~ q2gdp + juneapp + inc1 + mean.median,
                               data = National_Data_PlusIncome,
                               trControl = Train_Control,
                               #tuneGrid = Tune_Grid, for glmnet
                               method="glm")

#Info on model with mean.meadian measure
summary(National_Model_PlusIncome)
